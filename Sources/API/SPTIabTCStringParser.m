...
typedef NS_ENUM(NSInteger, SPTIabTCFSegmentType) {
    SegmentTypeCore = 0,
...
@property (assign, nonatomic) int offset;
@property (retain, nonatomic) NSArray<SPTIabPublisherRestriction *> *value;
...
+ (SPTIabTCFModel *)parseConsentString:(NSString *)consentString{
    ...
    SPTIabTCFModel *model = [SPTIabTCFModel new];
...
+ (void)completeModel:(SPTIabTCFModel *)model withCoreString:(NSString *)coreString {
    ...
    NSInteger version = [SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:VERSION_BIT_OFFSET length:VERSION_BIT_LENGTH];
    model.version = version;
    model.created = [[self class] decodeDateFromBinary:binaryCharBuffer fromIndex:CREATED_BIT_OFFSET length:CREATED_BIT_LENGTH];
    model.lastUpdated = [[self class] decodeDateFromBinary:binaryCharBuffer fromIndex:LAST_UPDATED_BIT_OFFSET length:CREATED_BIT_LENGTH];
    model.cmpId = [SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:CMP_ID_BIT_OFFSET length:CMP_ID_BIT_LENGTH];
    model.cmpVersion = [SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:CMP_VERSION_BIT_OFFSET length:CMP_VERSION_BIT_LENGTH];
    model.consentScreen = [SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:CONSENT_SCREEN_BIT_OFFSET length:CONSENT_SCREEN_BIT_LENGTH];
    model.consentCountryCode = [[self class] decode6BitCharStringFromBinary:binaryCharBuffer fromIndex:CONSENT_LANGUAGE_BIT_OFFSET length:CONSENT_LANGUAGE_BIT_LENGTH];
...
+ (void)completeModel:(SPTIabTCFModel *)model forType:(SPTTcfDecoderVendorStringType)type withVendorsString:(NSString *)vendors {
    ...
    SPTIabTCFSegmentType segmentType = [SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:0 length:SEGMENT_TYPE_BIT_LENGTH];
    SPTDecodedStringAndOffset *consentAndOffset;
    if (type == DecodeStringVendorsDisclosed) {
        if (segmentType != SegmentTypeDisclosedVendors) {
            return;
        }
        consentAndOffset = [[self class] decodeVendorBitChainStringFromBinary:binaryCharBuffer fromIndex:SEGMENT_TYPE_BIT_LENGTH version1:NO];
...
+ (void)completeModel:(SPTIabTCFModel *)model withPublisherPurposesTransparencyConsent:(NSString *)segmentString {
    ...
    model.publisherTCParsedPurposesConsents = [[self class] decodeBitChainStringFromBinary:binaryCharBuffer fromIndex:PUBLISHER_PURPOSES_CONSENT_BIT_OFFSET length:PUBLISHER_PURPOSES_CONSENT_BIT_LENGTH];
    model.publisherTCParsedPurposesLegitmateInterest = [[self class] decodeBitChainStringFromBinary:binaryCharBuffer fromIndex:PUBLISHER_PURPOSES_LEGIT_INTEREST_BIT_OFFSET length:PUBLISHER_PURPOSES_LEGIT_INTEREST_BIT_LENGTH];
    
    int variableOffset = PUBLISHER_NUM_CUSTOM_PURPOSES_BIT_OFFSET;
    NSInteger numCustomPurposes = [SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:PUBLISHER_NUM_CUSTOM_PURPOSES_BIT_OFFSET length:PUBLISHER_NUM_CUSTOM_PURPOSES_BIT_LENGTH];
...
+ (NSDate *)decodeDateFromBinary:(unsigned char*)binaryCharBuffer fromIndex:(int)startIndex length:(int)length {
    NSInteger deciSecondsEpoch = [SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:startIndex length:length];
    
    return [NSDate dateWithTimeIntervalSince1970:deciSecondsEpoch/10];
}

+ (NSString *)decode6BitCharStringFromBinary:(unsigned char*)binaryCharBuffer fromIndex:(int)startIndex length:(int)length {
    int a = 65; //first char
    int charBitLenght = 6;
    ...
    for (int i = 0; i<characterNumber; i++) {
        NSInteger letter = a + [SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:startIndex + charBitLenght*i length:charBitLenght];
...
+ (NSString *)decodeBitChainStringFromBinary:(unsigned char*)binaryCharBuffer fromIndex:(int)startIndex length:(int)length {
...
+ (SPTDecodedStringAndOffset *)decodeVendorBitChainStringFromBinary:(unsigned char*)binaryCharBuffer fromIndex:(int)startIndex version1:(BOOL)v1 {
    
    int totalOffset = startIndex;
    
    NSInteger maxVendorId = [SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:startIndex length:MAX_VENDOR_ID_BIT_LENGTH];
    totalOffset += MAX_VENDOR_ID_BIT_LENGTH;
    
    SPTDecodedStringAndOffset *vendorVLDS = [[self class] decodeBitChainStringFromBinary:binaryCharBuffer fromIndex:totalOffset entryNumber:maxVendorId version1:v1];
...
+ (SPTDecodedPublisherRestrictionAndOffset *)decodePublisherRestrictionBitChainStringFromBinary:(unsigned char*)binaryCharBuffer fromIndex:(int)startIndex {
    
    int totalOffset = startIndex;
    
    NSInteger numPubRestriction = [SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:startIndex length:NUM_PUBLISHER_RESTRICTIONS_BIT_LENGTH];
    ...
    for (int i=0; i<numPubRestriction; i++) {
        NSInteger purposeId = [SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:totalOffset length:PUBLISHER_RESTRICTIONS_PURPOSE_ID_BIT_LENGTH];
...
+ (unsigned char *)binaryFromString:(NSString *)consentString {
    NSString* safeString = [SPTIabTCFUtils safeBase64ConsentString:consentString];
    ...
    return [SPTIabTCFUtils NSDataToBinary:decodedData];
}
